...
func TestReadDir(t *testing.T) {
	...
	t.Run("read value from file", func(t *testing.T) {
		_, err := readValinFile("/dev", "nul=l")
		require.ErrorIs(t, err, ErrWrongVarName)

		_, err = readValinFile("/etc", "nonexistentfile123")
		require.ErrorIs(t, err, ErrUnableToOpenFile)

		result, err := readValinFile("testdata/env", "BAR")
		...
		result, err = readValinFile("testdata/env", "EMPTY")
		require.NoError(t, err)
		require.Equal(t, EnvValue{
			Value:      "",
			NeedRemove: true,
		...
		result, err = readValinFile("testdata/env", "FOO")
		require.NoError(t, err)
		require.Equal(t, EnvValue{
			Value:      "   foo\nwith new line",
			NeedRemove: false,
		...
		result, err = readValinFile("testdata/env", "HELLO")
		require.NoError(t, err)
		require.Equal(t, EnvValue{
			Value:      "\"hello\"",
			NeedRemove: false,
		...
		result, err = readValinFile("testdata/env", "UNSET")
		require.NoError(t, err)
		require.Equal(t, EnvValue{
			Value:      "",
			NeedRemove: true,
