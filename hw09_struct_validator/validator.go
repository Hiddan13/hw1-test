...
func Validate(v interface{}) error {
	...
	for i, f := range reflect.VisibleFields(typ) {
		...
		case []string:
			err = validateSliceString(val, tagValue, name)
		case int:
			err = validateInt(val, tagValue, name)
		case []int:
			err = validateSliceInt(val, tagValue, name)
		}
...
func containSlice(s []string, val int) (bool, error) {
	for _, v := range s {
...
func checkSlice(s []string, name string) error {
	if len(s) != 2 {
...
func validateString(val string, tagValue string, name string) error {
	...
	for _, v := range s {
		m := strings.Split(v, ":")
		if err := checkSlice(m, name); err != nil {
			return err
		}
		switch m[0] {
		case "len":
...
func validateInt(val int, tagValue string, name string) error {
	...
	for _, v := range s {
		...
		case "in":
			if ok, err := containSlice(strings.Split(m[1], ","), val); err != nil {
				return err
			} else if !ok {
...
func validateSliceInt(val []int, tagValue string, name string) error {
	var ve ValidationErrors
	for _, v := range val {
...
func validateSliceString(val []string, tagValue string, name string) error {
	var ve ValidationErrors
	for _, v := range val {
